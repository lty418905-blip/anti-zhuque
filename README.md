...
仓库名保留为 `anti-zhuque`，本项目旨在使得LLM生成文本提高对朱雀AIGC检测的规避能力。

## 完整能力范围

嵌入的 Human Writing 内容包括：
...
若用户提供检测报告，审计阶段还要再分两步：先不看分数和定位完成全文冷读，只锁定一个主病灶；再核验报告对应的输入、格式和分段边界。单次报告只提供复看位置，不能直接触发删句。局部问题可以一次合上时才做有界修订；分布式声线或结构性问题必须转作者级完整重写，不能用删一两句冒充修复。
...
- 修订后的分数升降不能自动归因于某句话；输入、格式、分段和检测端变化必须分开处理。
